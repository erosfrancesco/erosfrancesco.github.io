const GoLCell = (x = 0, y = 0, cellProperties = []) => {
    this.x = x;
    this.y = y;
    this.state = {};
...
const GoLLayer = (width = 100, height = 100, cellPropertiesDistribution = function(x, y) { return []; }) => {
    this.width = width;
    this.height = height;
    this.cells = [[]];
    this.rules = [];
    ...
    this.cellIterator = (iteratee = function(cell) { }) => 
        this.cells.forEach((row, y) => row.forEach((cell, y) => iteratee( this.getCell(x, y) ))
    );
    ...
    this.addRule = (id, rule) => this.rules.push({id, rule});
    this.removeRule = id => {
    ...
    this.update = () => {
        ...
        buffer.forEach(updatedCell => {
            const cell = this.getCell(cell.x, cell.y);
